# 文档导航

| 主题 | 文档 |
|---|---|
| 产品定位与目标 UX | [MVP 产品与 UX](MVP_PRODUCT_UX.md) |
| 当前桌面能力与使用 | [桌面应用](DESKTOP_APP.md) |
| 模块职责与依赖方向 | [仓库架构](architecture/REPOSITORY.md) |
| 构建产物与发布门槛 | [发布流程](engineering/RELEASE.md) |
| 工程整理验收 | [2026-09-19](engineering/VALIDATION-2026-09-19.md) |
| 差距修复验收 | [2026-09-20](engineering/VALIDATION-2026-09-20-GAPS.md) |
| Linux 客户端验收 | [2026-09-21](engineering/VALIDATION-2026-09-21-LINUX.md) |
| Linux 客户端设计与计划 | [设计](superpowers/specs/2026-09-20-linux-client-design.md)、[实施计划](superpowers/plans/2026-09-21-linux-client.md) |
| 改名迁移 | [Qisuo → Anchi](engineering/RENAME_MIGRATION.md) |
| 变更记录 | [CHANGELOG](../CHANGELOG.md) |
| English summary | [README.en.md](../README.en.md) |
| 开发入口和测试 | [贡献约定](../CONTRIBUTING.md) |
| 安全模型 | [安全声明](../SECURITY.md)、[安全基础](SECURITY_FOUNDATION.md) |
| VM 与 cell 设计 | [架构设计](../SECURE_VM_DESIGN.md) |
| Pi 安装与认证 | [Pi agent](PI_AGENT.md) |
| Pi 聊天与 RPC v1 | [通信接口](PI_CHAT.md) |
| Gmail 只读接入 | [Gmail 配置](GMAIL_SETUP.md) |
| 连接器验收 | [2026-09-21](engineering/VALIDATION-2026-09-21-CONNECTORS.md) |
| Drive / Notion / Slack 连接器 | [连接器](CONNECTORS.md)、[设计](superpowers/specs/2026-09-21-connectors-design.md)、[实施计划](superpowers/plans/2026-09-21-connectors.md) |
| 早期工作流（历史） | [旧受限工作流](AGENT_WORKFLOW.md) |
| 历史验收证据 | [2026-09-18](VALIDATION-2026-09-18.md) |

UX 文档描述目标体验；以桌面能力表与安全文档判断当前实际保证。历史验收只适用于其记录的版本和范围。

- [首次使用：安装到首个 Pi 任务](GETTING_STARTED.md)
