...
def main():
    ...
    parser.add_argument('--allow-cloud-mail', action='store_true',
    ...
    if not args.disable:
        ...
        data = {'provider': 'openai', 'model': args.model,
                'api_key': args.key_file.read_text().strip(), 'allow_cloud_mail': True}
    proc = subprocess.run(['limactl', 'shell', 'secure-vm', '--', 'sudo', '/usr/bin/python3',
        '/opt/secure-vm/services/model_admin.py', 'disable' if args.disable else 'configure'],
        input='' if data is None else json.dumps(data), text=True, capture_output=True)
    if proc.returncode:
