...
export function receiveHostResult(value) {
  ...
  if (item) { pending.delete(value.id); item.finish(value); }
  return true;
}
export function hostFileRequest(request, notify, signal) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const abort = () => { cleanup(); reject(Error('ABORTED')); };
    const timer = setTimeout(() => { cleanup(); reject(Error('HOST_FILE_TIMEOUT')); }, 30000);
    pending.set(id, { finish: value => { cleanup(); value.ok ? resolve(value.result) : reject(Error(value.error || 'FILE_OPERATION_DENIED')); } });
