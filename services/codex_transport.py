...
def read_response(response):
    if response.status != 200:
        code = {401:'CODEX_AUTH_REQUIRED',403:'CODEX_ACCESS_DENIED',429:'CODEX_USAGE_LIMIT'}.get(response.status, 'CODEX_HTTP_' + str(response.status))
        body = response.read(16384).lower()
        for term, label in [(b'model', 'MODEL'), (b'unsupported parameter', 'UNSUPPORTED_PARAMETER'),
                            (b'instructions', 'INSTRUCTIONS'), (b'stream', 'STREAM'), (b'tool', 'TOOL'),
                            (b'reasoning', 'REASONING'), (b'input', 'INPUT'), (b'cookie', 'COOKIE')]:
            if term in body:
                code += '_' + label
        raise Denied(code)
...
def normalize_response(result):
    ...
    if len(json.dumps(result, ensure_ascii=True).encode()) > 48000:
        raise Denied('CODEX_RESPONSE_TOO_LARGE')
    return result
...
def responses(payload, credential):
    ...
    try:
        ...
        connection.request('POST','/backend-api/codex/responses',body=json.dumps(payload).encode(),headers={
            'Authorization':'Bearer '+credential['access_token'],
            'chatgpt-account-id':credential['account_id'], 'originator':'pi',
            'User-Agent':'pi/0.85.1 secure-vm', 'OpenAI-Beta':'responses=experimental',
