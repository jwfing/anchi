...
def as_user(user, code, data=None):
    run = subprocess.run(['runuser', '-u', user, '--', 'python3', '-c',
...
def rpc_as(user, path, request):
    return as_user(user, "from common import rpc, Denied; import json\ntry:\n print(json.dumps(rpc(" + repr(path) + ", json.load(sys.stdin))))\nexcept Denied as e:\n print(json.dumps({'error':str(e)}))", request)

check('no_swap', len(Path('/proc/swaps').read_text().splitlines()) == 1)
check('no_plaintext_credentials', not any((Path('/var/lib/secure-auth') / n).exists() for n in ('client.json','tokens.json','pending.json','model.json')))
check('vault_key_tmpfs', subprocess.check_output(['findmnt','-n','-o','FSTYPE','-T','/run/secure-vault'], text=True).strip() == 'tmpfs')
...
for user, host in [('secure-auth','oauth2.googleapis.com'), ('secure-gmail','gmail.googleapis.com'), ('secure-inference','api.openai.com'), ('secure-inference','chatgpt.com')]:
    probe = "from common import target_ips; import socket, ssl, json; host=" + repr(host) + "; raw=socket.create_connection((target_ips(host)[0],443),timeout=5); tls=ssl.create_default_context().wrap_socket(raw,server_hostname=host); print(json.dumps(bool(tls.version()))); tls.close()"
    check(user + '_' + host + '_tls_allowed', as_user(user, probe))
check('gmail_cannot_get_codex_token', rpc_as('secure-gmail','/run/secure-auth/token.sock',{'op':'codex_token'}).get('error') == 'CREDENTIAL_SCOPE_DENIED')
check('gmail_cannot_get_model_key', rpc_as('secure-gmail','/run/secure-auth/token.sock',{'op':'model_key'}).get('error') == 'CREDENTIAL_SCOPE_DENIED')
check('inference_cannot_get_google_token', rpc_as('secure-inference','/run/secure-auth/token.sock',{'op':'access_token'}).get('error') == 'CREDENTIAL_SCOPE_DENIED')
check('service_cannot_approve', rpc_as('secure-inference','/run/secure-policy/api.sock',{'op':'approve'}).get('error') == 'OPERATION_DENIED')
# Synthetic action only: no inference execution and no cloud request.
action = {'operation':'inference.openai','account':'synthetic-security-test','params':{
    'model':'test-model','instructions':'synthetic approval test','input':[],
    'max_output_tokens':1,'store':False,'tools':[],'stream':False}}
...
subprocess.run(['python3','/opt/secure-vm/services/policy_admin.py','approve',pending['approval_id'],'--digest',pending['digest']], check=True, capture_output=True)
