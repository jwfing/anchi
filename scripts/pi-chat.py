        ...
        try:
            for line in process.stdout:
                ...
                if kind == 'ready':
                    ...
                    if args.resume:
                        resume_id = send('resume', session_id=args.resume)
                    else:
                        ready.set()
                    ...
                    print(
                        '\n待审批：'
                        + safe(approval)
                        + '\n请在另一个终端运行：bash scripts/policy.sh show '
                        + safe(approval),
                        flush=True,
                    )
                    ...
                    if event.get('id') == resume_id:
                        ...
                        ready.set()
                elif kind in ('protocol_error', 'turn_error'):
                    print('\n[错误] ' + safe(event.get('error')), flush=True)
        finally:
            ready.set()
    ...
    try:
        ...
        while process.poll() is None:
            ...
            if text.startswith('/resume '):
                send('resume', session_id=text.split(maxsplit=1)[1])
            elif text in ('/status', '/sessions', '/history', '/new', '/cancel'):
                send(text[1:])
