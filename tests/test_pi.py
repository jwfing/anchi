...
class PiTests(unittest.TestCase):
    def setUp(self):
        ...
        self.patches=[patch('pi_gateway.CONFIG',self.config),patch('inference.DATABASE',self.root/'runs.sqlite3')]
        for p in self.patches:p.start()
        self.request={'op':'pi_generate','request_id':'a'*32,'instructions':'Test',
                      'input':[{'role':'user','content':[{'type':'input_text','text':'test'}]}], 'tools':[]}
        self.credential={'access_token':'SECRET','account_id':'account','generation':'generation','expires_at':time.time()+3600}
        self.result={'id':'response1','status':'completed','output':[{'type':'message','content':[{'type':'output_text','text':'OK'}]}]}
    def tearDown(self):
    ...
    def test_approval_before_network_and_resume(self):
        with patch('policy_client.require',side_effect=Denied('APPROVAL_REQUIRED:id')),patch('codex_transport.responses') as network:
            with self.assertRaisesRegex(Denied,'APPROVAL_REQUIRED'):self.call()
            network.assert_not_called()
        with patch('policy_client.require') as permit,patch('codex_transport.responses',return_value=self.result) as network:
    ...
    def test_changed_input_conflicts(self):
        with patch('policy_client.require'),patch('codex_transport.responses',return_value=self.result):
            ...
            with self.assertRaisesRegex(Denied,'REQUEST_ID_CONFLICT'):self.call()
    def test_interrupted_network_is_not_retried(self):
        with patch('policy_client.require'),patch('codex_transport.responses',side_effect=TimeoutError) as network:
            with self.assertRaisesRegex(Denied,'CODEX_EXECUTION_UNKNOWN'):self.call()
            with self.assertRaisesRegex(Denied,'REQUEST_ALREADY_UNKNOWN'):self.call()
            self.assertEqual(network.call_count,1)
    ...
    def test_no_hosted_tools_or_remote_content(self):
        for tool in ({'type':'web_search','name':'read','parameters':{}}, {'type':'function','name':'gmail_send','parameters':{}}):
            with self.assertRaises(Denied):codex_schema.validate_parts('test',self.request['input'],[tool])
        for kind in ('input_image','input_file'):
            with self.assertRaises(Denied):codex_schema.validate_parts('test',[{'role':'user','content':[{'type':kind,'image_url':'https://evil.test'}]}],[])
    def test_desktop_file_tool_reaches_approval_with_full_tool_set(self):
        self.request['tools'] = [{'type': 'function', 'name': name, 'parameters': {'type': 'object'}} for name in sorted(codex_schema.TOOL_NAMES)]
        self.assertEqual(len(self.request['tools']), 8)
        with patch('policy_client.require', side_effect=Denied('APPROVAL_REQUIRED:id')), patch('codex_transport.responses') as network:
            with self.assertRaisesRegex(Denied, 'APPROVAL_REQUIRED'):
                self.call()
            network.assert_not_called()
        codex_schema.validate_parts('test', [{'type':'function_call','call_id':'c1','name':'host_files','arguments':'{}'}], self.request['tools'])
        with self.assertRaises(Denied):
            codex_schema.validate_parts('test', self.request['input'], self.request['tools'] + [self.request['tools'][0]])

    def test_oversized_context_denied(self):
        self.request['input'][0]['content'][0]['text']='x'*45000
        with self.assertRaisesRegex(Denied,'CONTEXT_TOO_LARGE'):self.call()
    def test_model_daily_limit(self):
        ...
        with patch('codex_transport.responses') as network:
            with self.assertRaisesRegex(Denied,'DAILY_REQUEST_LIMIT'):self.call()
            network.assert_not_called()
    def test_subscription_token_import_validation(self):
        ...
        for change in ({'account_id':'other'},{'access_token':'sk-'+'x'*100},{'refresh_token':'DO_NOT_COPY'}):
            with self.assertRaises(Denied):codex_admin.validate({**value,**change})
        claims['exp']=0
        value['access_token']='header.'+base64.urlsafe_b64encode(json.dumps(claims).encode()).decode()+'.signature'
        with self.assertRaisesRegex(Denied,'EXPIRED'):codex_admin.validate(value)
    def test_sse_done_with_missing_content_type(self):
        class Response(io.BytesIO):
            status=200
            def getheader(self,*args):return ''
    ...
    def test_sse_incomplete_never_success(self):
        ...
        with self.assertRaises(RuntimeError):codex_transport.read_response(Response(b'data: [DONE]\n'))
    ...
    def test_multiple_provider_allow_rules_precede_reject(self):
        class User:
            def __init__(self,uid):self.pw_uid=uid
        users={'secure-auth':1,'secure-gmail':2,'secure-inference':3,'secure-policy':4}
        with patch('network_rules.pwd.getpwnam',side_effect=lambda name:User(users[name])),patch('network_rules.subprocess.run') as run,patch('network_rules.nft') as nft:
