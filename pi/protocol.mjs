...
export function publicEvent(event) {
  ...
  if (event.type === 'tool_execution_end')
    return { type: 'tool_end', tool: event.toolName, is_error: event.isError };
...
export class Controller {
  ...
  info() {
    ...
    return {
      ...
      busy: !!this.active,
      turn_id: this.active?.id ?? null,
    };
  }
  emit(event) {
    this.notify({
      ...event,
      session_id: this.current?.session.sessionId,
      turn_id: this.active?.id ?? null,
    });
  }
  ...
  response(id, op, ok, result) {
    this.notify({ type: 'response', id, op, ok, ...(ok ? { result } : { error: result }) });
  }
  async handle(request) {
    ...
    try {
      ...
      if (Object.keys(request).some((k) => !['id', 'op', ...extra].includes(k)))
        throw new Error('BAD_REQUEST');
      if (this.closed) throw new Error('CONNECTION_CLOSED');
      if (op === 'status') return this.response(id, op, true, this.info());
      if (op === 'sessions')
        return this.response(id, op, true, { sessions: await this.listSessions() });
      if (op === 'cancel') {
      ...
      if (op === 'close') {
        await this.close();
        return this.response(id, op, true, { closed: true });
      }
      if (this.active) throw new Error('BUSY');
      if (op === 'new' || op === 'resume') {
        if (op === 'resume' && typeof request.session_id !== 'string')
      ...
      if (
        ...
        !request.text.trim() ||
        request.text.length > LIMITS.prompt_chars
      )
        throw new Error('BAD_PROMPT');
      ...
      turn.done = Promise.resolve().then(async () => {
        const timeout = setTimeout(() => {
          turn.cancelled = true;
          void this.current.session.abort();
        }, this.timeoutMs);
        try {
          await this.current.session.prompt(request.text);
        } catch {
          turn.failed = true;
          this.emit({ type: 'turn_error', error: 'PI_EXECUTION_FAILED' });
        } finally {
          clearTimeout(timeout);
          this.emit({
            type: 'finished',
            success: !turn.failed && !turn.cancelled,
...
export async function serve({ input, notify, ...options }) {
  ...
  const enqueue = (line) => {
    if (queued >= 32) {
      notify({ type: 'protocol_error', error: 'COMMAND_QUEUE_FULL' });
      return;
    }
    queued++;
    queue = queue
      .then(async () => {
        ...
        try {
          ...
          notify({ type: 'protocol_error', error: 'INVALID_JSON' });
          return;
        }
  ...
  try {
    for await (const chunk of input) {
      for (const part of decoder.write(chunk).split(/(?<=\n)/)) {
        ...
        if (Buffer.byteLength(buffer) > LIMITS.rpc_bytes) {
          buffer = '';
          discarded = true;
          notify({ type: 'protocol_error', error: 'LINE_TOO_LARGE' });
        }
        if (part.endsWith('\n')) {
