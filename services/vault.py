...
def decrypt(name, envelope, master):
    try:
        if envelope['version'] != 1:
            raise ValueError()
        plaintext = AESGCM(master).decrypt(base64.b64decode(envelope['nonce'], validate=True),
            base64.b64decode(envelope['ciphertext'], validate=True), ('secure-vm:v1:' + name).encode())
...
def write(store, name, value):
    ...
    envelope = {'version': 1, 'nonce': base64.b64encode(nonce).decode(),
