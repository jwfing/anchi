...
class Client:
    def __init__(self):
        ...
        self.process = subprocess.Popen(
            ['bash', str(root / 'scripts/pi.sh'), '--rpc'],
...
try:
    ...
    if args.live:
        ...
        for prompt in [
            f'Remember this marker for our conversation: {marker}. Reply READY only. Do not use tools.',
        ...
        response = client.call(
            'prompt', text='After reopening this session, repeat the remembered marker only. Do not use tools.'
        )
        finished = client.wait(
            lambda e: e.get('type') == 'finished' and e.get('turn_id') == response['result']['turn_id'], timeout=600
        )
