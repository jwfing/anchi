...
可写目录不能覆盖当前应用代码、运行资源、宿主解释器及已发现工具的安装路径，也不能选中包含它们的父目录。启动恢复旧授权时执行同样检查，冲突授权保持未激活；用户可移除或改为只读。宿主系统目录与凭证目录继续禁止授权。

本地文本覆盖先复制旧内容到私有回收目录并同步，再用原子替换发布新文件。替换失败时原路径保留；已有内容超过 24 KB 时拒绝覆盖。删除仍移入回收目录。回收项使用固定长度 ID，配套 `.json` 记录原始相对路径，恢复由用户在宿主进行。

旧策略库迁移不会把读取许可扩大为写入许可，所有主体先采用逐次审批；已经设置的新版模式保持不变。连接器审批等待保留相同请求，写入动作首次冻结后持久化。远端结果不明确时记录 UNKNOWN 并禁止同 ID 重放。Drive 文本覆盖要求强 ETag / 修订号并发送 If-Match；Google 文档覆盖暂时拒绝。Notion 编辑时间检查不构成原子并发控制。真实远端条件写入、Linux 隔离和签名公证仍须按发布流程验收。
