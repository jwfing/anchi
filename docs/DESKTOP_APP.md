...
4. 回到应用刷新状态。连接账户不会自动扩大 Agent 权限；如需要，单独点击「允许 Agent 持续只读」并确认。原先已有的持续许可仍按既有策略生效。
5. 「撤销持续读取许可」撤销全部待处理策略记录，以后邮件请求需逐次审批。「断开账户」还停止 Pi 并撤销 Google 令牌。若远端撤销失败，显示待重试，再次点击断开即可。

可取消等待中的 OAuth；十分钟内未完成自动取消。真实交互式授权必须由账户所有者完成。模型仍逐请求审批，任务级邮件范围、预算界面及自动更新尚未实现。
...
4. 发送任务。收到提示后进入「独立审批」，点击「从策略服务刷新」。
5. 查看完整请求，核对实际内容、模型、资源及 digest；点击批准后在原生对话框确认。每一轮新模型请求可能再次需要审批。
