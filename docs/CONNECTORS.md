...
2026-09-21。四个 connector 都在 `services/connectors.py` 注册，各自独立 UID、独立 socket、独立出口；读取受持续只读许可或逐次审批控制，写入永远逐条审批。凭证只进入 VM 凭证库，cell 内的 agent 只能通过结构化操作调用。
...
写入正文上限 48 KB；每个 connector 每天最多 200 次写入。Pi 只为「已连接」的 connector 注册工具。
