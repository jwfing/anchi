...
| Google Drive / Notion / Slack 连接器 | 已实现：连接即持续授权（读写自动放行），可按 connector 改为逐次审批；更新类写入绑定目标修订；真实账户验证待完成 |
...
| 桌面 Gmail OAuth、账户断开、授权模式切换、需要重新认证状态 | 已实现；浏览器交互式授权需用户完成 |
| 首次引导、依赖安装、VM/Pi 安装、解锁与订阅认证 | 已接入桌面；认证到期可直接重新导入，不必断开 Pi；全新 VM 安装与重试通过 |
| 首个示例任务与审批引导 | 已实现；模型调用默认持续授权，可改为逐轮审批 |
