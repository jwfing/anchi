...
install -d -m 0755 /opt/secure-vm
...
if [[ ! -f "$base/rootfs-ready" ]]; then
  ...
  chroot "$root" /usr/sbin/useradd --uid "$SECURE_CELL_AGENT_UID" --user-group --home-dir /workspace --shell /bin/bash agent
  ...
  systemd-nspawn --quiet --register=no --settings=no --directory="$root" \
    --private-users="$SECURE_CELL_UID_BASE:$SECURE_CELL_UID_COUNT" --private-users-ownership=chown \
    --private-network /bin/true
  touch "$base/rootfs-ready"
  rm -f "$base/rootfs-installing"
fi
install -d -o "$agent_host_uid" -g "$agent_host_uid" -m 0700 "$base/workspace"
install -d -o "$SECURE_CELL_UID_BASE" -g "$SECURE_CELL_UID_BASE" -m 0755 "$root/opt/secure-vm"
install -o "$SECURE_CELL_UID_BASE" -g "$SECURE_CELL_UID_BASE" -m 0644 "$src/check-cell.py" "$root/opt/secure-vm/check-cell.py"
install -o "$SECURE_CELL_UID_BASE" -g "$SECURE_CELL_UID_BASE" -m 0644 "$src/cell.env" "$root/opt/secure-vm/cell.env"
...
printf 'Bootstrap complete. Rootfs: %s; mapped agent UID: %s\n' "$root" "$agent_host_uid"
