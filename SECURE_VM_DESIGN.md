...
第一版使用确定性规则：读取按已授予范围放行；发送逐次审批；未知操作默认拒绝。模型可以提供解释，但不能签发授权、降低风险级别或代替用户点击确认。
...
| 决策 | 当前提案 | 状态 |
|---|---|---|
| 部署环境 | Lima + VZ，Ubuntu 24.04 arm64，macOS 宿主 | 已在本机启动 |
| Runtime | systemd-nspawn，普通用户进程；尚未接 harness | 隔离原型已通过首轮验收 |
| 开发语言 | 未定；统一服务语言优先 | 待讨论 |
| MVP 出口 | 仅结构化 Gmail 与固定推理服务 | 待确认 |
| 凭证传递 | 不向 cell/worker 提供 token；暂不做通用 surrogate | 待确认 |
| 发送授权 | 每封明确审批、对象冻结、一次性消费 | 待确认 |
| 审批客户端 | 独立终端或本地 Web | 待讨论 |
| 状态存储 | 可信状态服务独占 SQLite | 待确认 |
| 高级能力 | 浏览器、eBPF taint、多用户延后 | 待确认 |
