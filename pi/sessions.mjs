...
export async function sessionPath(id, directory = DIRECTORY) {
  ...
  try {
    ...
    if (header.type !== 'session' || header.id !== id || header.cwd !== '/workspace')
      throw new Error('INVALID_SESSION_FILE');
  } finally {
    await handle.close();
  }
...
export async function listSessions(directory = DIRECTORY) {
  ...
  for (const name of files.sort().reverse().slice(0, 100)) {
    ...
    if (!id) continue;
    try {
      const file = await sessionPath(id, directory);
      const stat = await fs.stat(file);
      result.push({ session_id: id, session_file: file, modified: stat.mtime.toISOString() });
    } catch {
      /* Invalid workspace data is not a resumable session. */
    }
  }
  return result;
}
