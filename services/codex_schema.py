...
def validate_parts(instructions, inputs, tools):
    ...
    if not isinstance(inputs, list) or not 1 <= len(inputs) <= 100 or not isinstance(tools, list) or len(tools) > len(TOOL_NAMES):
    ...
    for item in inputs:
        ...
        if kind == 'message':
            ...
            for content in item['content']:
                ...
                if content['type'] not in ('input_text','output_text') or not isinstance(content['text'], str) or content.get('annotations', []) != []:
                    raise Denied('TEXT_ONLY')
        elif kind == 'function_call':
            fields(item, ('type','id','call_id','name','arguments','status'), ('type','call_id','name','arguments'))
            ...
            raise Denied('BAD_CODEX_INPUT')
    if len(json.dumps([instructions,inputs,tools], ensure_ascii=True).encode()) > 44000:
        raise Denied('CODEX_CONTEXT_TOO_LARGE')

def validate_payload(payload):
    fields(payload, ('model','store','stream','instructions','input','tools','tool_choice','parallel_tool_calls','include','reasoning'),
           ('model','store','stream','instructions','input','tools','tool_choice','parallel_tool_calls','include','reasoning'))
    if not isinstance(payload['model'], str) or not re.fullmatch('gpt-[a-zA-Z0-9._-]{1,80}', payload['model']):
        raise Denied('BAD_MODEL_NAME')
    if payload['store'] is not False or payload['stream'] is not True or payload['parallel_tool_calls'] is not False or payload['tool_choice'] != 'auto' or payload['include'] != ['reasoning.encrypted_content'] or payload['reasoning'] != {'effort':'low','summary':'auto'}:
